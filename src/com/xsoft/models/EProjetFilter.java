...
public class EProjetFilter implements Filter {
	...
	public void init(FilterConfig filterConfig) throws ServletException {
		...
		if (ormRequest == null || ormRequest.equals("false")) {
			...
			if (System.getProperty("weblogic.Name") == null)  {
				...
				try {
					servletRequestFactory = new ORMServletRequestFactory(EProjetPersistentManager.instance());
				}
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain filterChain) throws IOException, ServletException {
		...
		try {
			...
			try {
				PersistentSession session = EProjetPersistentManager.instance().getSession(false);
				if (session != null) {
					session.close();
				}
			}
